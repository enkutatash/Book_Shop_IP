        ...
        if (isset($_FILES["book"]) && $_FILES["book"]["error"] == 0) {
            ...
            if (move_uploaded_file($_FILES["book"]["tmp_name"], $target_file)) {
                ...
                if (move_uploaded_file($coverimagetemp, $cover_target_file)) {
                    ...
                    $sql = $conn->prepare("UPDATE textbook SET bookname = :bookname, price = :price, coverimage = :coverimage, coversize = :coversize, covertype = :covertype, blog = :blog, filename = :filename, filesize = :filesize, filetype = :filetype WHERE id = :id");                    $sql->bindParam(':bookname', $name);
                    ...
                    if ($result) {
                        ...
                        echo 'Error: Could not edit the book';
                        exit();
                    }
                } else {
